        ...
        <HomeLayout>
            <div className="flex flex-col h-full">
                ...
                <header className="h-[calc(6*var(--k-unit))] shrink-0 flex items-center px-8 gap-4">
                    ...
                    <h1 className="text-4xl font-black text-white">TÌM KIẾM</h1>
                </header>
                ...
                <div className="flex-1 overflow-auto px-8 pb-8 flex flex-col gap-8">
                    ...
                    <motion.div
                        ...
                        className="w-full h-[90px] glass-bar rounded-full flex items-center px-8 gap-4 shadow-lg"
                    >
                        <MagnifyingGlass size={32} className="text-white/60 shrink-0" />
                        <input
                            type="text"
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            placeholder="Tìm kiếm địa điểm, nhà hàng, khách sạn..."
                            className="flex-1 bg-transparent text-2xl font-medium text-white placeholder-white/40 focus:outline-none h-full"
                            autoFocus
                        />
                    </motion.div>
                    ...
                    {/* Categories Grid */}
                    <section>
                        <h2 className="text-2xl font-bold text-white/60 uppercase tracking-wider mb-4">
                            Danh mục
                        </h2>
                        <div className="grid grid-cols-3 gap-4">
                            {CATEGORIES.map((cat, index) => (
                                <motion.button
                                    ...
                                    className={`aspect-square rounded-3xl flex flex-col items-center justify-center gap-4 transition-all border-2 ${selectedCategory === cat.id
                        ...
                        <div className="flex flex-col gap-3">
                            {SUGGESTIONS.map((item, index) => (
                                <motion.div
                                    ...
                                    className="glass rounded-2xl p-5 flex items-center gap-5 cursor-pointer hover:bg-white/10 transition-colors"
                                >
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-lg text-white/60">{item.type}</p>
                                    </div>
                                    <div className="flex flex-col items-end gap-1">
                                        <span className="flex items-center gap-1 text-yellow-400 font-bold">
                                            <Star size={20} weight="fill" /> {item.rating}
                                        </span>
                                        <span className="text-white/40 text-sm">{item.distance}</span>
                                    </div>
                                </motion.div>
                            ))}
                        </div>
